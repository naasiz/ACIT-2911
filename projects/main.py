    ...
    if search is not None:
        for subheading in results:
            subheading.threads = [
                ...
                if search.lower() in thread.title.lower()
            ]
        results = [subheading for subheading in results if len(subheading.threads) != 0]
...
"""
No need to use 'own' flag as we can use thread_author_id = current_user.id directly in different function.

thread_detailed function: if users not login, just show forum.html(default) and thread_detailed (written in def thread_detailed() )
"""
...
def thread_detailed(thread_id):
    ...
    if current_user.is_authenticated:
        ...
        return render_template(
            "thread_detailed.html", thread=thread, edit=False, edit_comment_id=None
        )
    else:
...
def del_thread(thread_id):
    ...
    # Only allow delete for users who own thread and Admin
    if current_user.id == thread.author.id or current_user.id == 1:
        db.session.delete(thread)  # Delete the thread from the database
        db.session.commit()  # Commit the changes to the database
    return redirect(url_for("main.index"))  # Redirect to the index route
...
def thread_update(thread_id):
    ...
    # only allow own users to edit their own thread
    if current_user.id == thread.author.id:
        if request.form["title"] == "":  # Check if the title field is not empty
            # (FLASH HERE)
            return redirect(
...
@main.route(
    "/comment_reply/<int:comment_id>", methods=["POST"]
)  # Route decorator for the add_reply route
def add_reply(comment_id):
    parent_comment = db.get_or_404(
        Comment, comment_id
    )  # Get the parent comment with the specified comment_id from the database
    if request.form["content"] != "":  # Check if the content field is not empty
        try:
            user = db.get_or_404(
                User, current_user.id
            ...
            )  # Create a new reply with the author, thread, content, and parent_id
        except AttributeError:
            db.session.add(
                Comment(
                    thread=parent_comment.thread,
            ...
            )  # Create a new reply with the thread, content, and parent_id
        db.session.commit()  # Commit the changes to the database
    return redirect(
        url_for("main.thread_detailed", thread_id=parent_comment.thread.id)
    )  # redirect to thread_detailed.html
...
def comment_edit(comment_id):
    ...
    comment.count = (
        db.session.query(Comment).filter(Comment.parent_id == comment.id).count()
    ...
    )  # Render the thread_detailed.html template with the thread, comment, current user, and edit flag


@main.route(
    "/update/<int:comment_id>", methods=["POST"]
)  # Route decorator for the comment_update route
def comment_update(comment_id):
    ...
    if request.form["content"] == "":  # Check if the content field is not empty
        return redirect(
            url_for("main.thread_detailed", thread_id=comment.thread.id)
        )  # Redirect to the thread_detailed route
